...
scene("game", () => {
    ...
    // Right side: Instructions (Using anchor("topright") to align to the right edge)
    add([text("CONTROLS: Arrows to Move & Climb", { size: 18 }), pos(780, 20), anchor("topright")]);
    add([text("SPINACH: Grab to turn YELLOW and defeat Brutus!", { size: 18 }), pos(780, 50), anchor("topright"), color(50, 255, 50)]);
...
scene("over", (finalScore) => {
    ...
    // The visual instruction to restart
    const retryText = add([text("Press SPACE to Retry", { size: 24 }), pos(width() / 2, 400), anchor("center")]);
    
    // Makes the retry text blink!
    loop(0.5, () => {
        retryText.hidden = !retryText.hidden;
    });

    // The actual keypress that restarts the game
